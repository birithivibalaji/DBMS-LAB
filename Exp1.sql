SQL> Create table Student8(StudentID int PRIMARY KEY, Name varchar(30), Age int);

Table created.

SQL> Create table Courses8 (CourseID int PRIMARY KEY, CourseName varchar(20));

Table created.

SQL> Create table Enrollments8 (StudentID int REFERENCES Student(StudentID), CourseID int REFERENCES Courses(CourseID));
...
SQL> SELECT Name FROM Student8 WHERE StudentID IN (SELECT StudentID FROM Enrollments8 WHERE CourseID = (SELECT CourseID FROM Courses8 WHERE CourseName = 'Database Management'));
